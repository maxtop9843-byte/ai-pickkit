...
6. GitHub와 배포 플랫폼의 실제 원격 상태를 직접 확인한 뒤 작업 가능 여부와 effective status를 판정한다.
7. 연결 도구를 호출하지 않은 채 접근 불가, 권한 부족, 검증 불가 또는 실행 불가라고 추측해 보고하지 않는다.
8. 첫 실패나 빈 결과만으로 차단하지 않는다. 대상 식별자, 권한 범위와 합리적인 대체 조회 경로를 확인해 재시도한다.
9. 실제 조회와 재시도 후에도 진행할 수 없을 때만 차단으로 보고하며, 사용한 도구, 조회 대상과 확인된 오류를 구체적으로 기록한다.
10. 구체적인 도구명, 함수 호출 순서, 고정 project/team/deployment 식별자와 환경별 복구 절차는 이 문서가 아니라 해당 실행 프롬프트에서 관리한다.
